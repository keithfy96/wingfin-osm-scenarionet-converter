...
class CarStateBase(ABC):
  ...
  def update_speed_kf(self, v_ego_raw):
    predicted_x0 = self.v_ego_kf.x[0][0] * 1.0 + self.v_ego_kf.x[1][0] * DT_CTRL
    predicted_x1 = self.v_ego_kf.x[1][0] * 1.0

    # Check for a complete reset condition (vehicle stationary for a prolonged period).
    # Trigger a reset if raw speed is near zero and the filtered speed is also near zero.
    if abs(v_ego_raw) < 0.05 and abs(self.v_ego_kf.x[0][0]) < 0.05:
      # Re-create the KF1D object to reset all its internal state.
      self.v_ego_kf = KF1D(
        x0=[[0.0], [0.0]],
        A=[[1.0, DT_CTRL], [0.0, 1.0]],
        C=[1.0, 0.0],
      ...
      return float(self.v_ego_kf.x[0][0]), float(self.v_ego_kf.x[1][0])
    elif abs(v_ego_raw - predicted_x0) > 2.0:
      # Ignore the bad measurement. The filter's next state becomes the prediction.
      self.v_ego_kf.x = [[predicted_x0], [predicted_x1]]
      return float(predicted_x0), float(predicted_x1)

      # Normal case: perform the Kalman filter update with the valid measurement.
    else:
