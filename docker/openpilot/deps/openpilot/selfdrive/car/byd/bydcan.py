...
def create_wheelspeed_spoof_command(packer, raw_cnt):
  values = {
    "WHEELSPEED_FL": 0,
    "WHEELSPEED_FR": 0,
    "WHEELSPEED_BL": 0,
  ...
  dat = packer.make_can_msg("WHEEL_SPEED", 0, values)[2]
  crc = byd_checksum(0xaf, dat[:-1])
  values["CHECKSUM"] = crc
  return packer.make_can_msg("WHEEL_SPEED", 0, values)


def create_can_steer_command(packer, steer_angle, steer_req, is_standstill, ecu_fault, recovery_btn, raw_cnt):
...
def create_lkas_hud(packer, enabled, lss_state, lss_alert, tsr, ahb, passthrough,\
    hma, pt2, pt3, pt4, pt5, lka_on, raw_cnt):
