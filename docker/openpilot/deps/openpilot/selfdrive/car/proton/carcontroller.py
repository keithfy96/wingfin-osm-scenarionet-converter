...
class CarController():
  ...
  def update(self, CC, CS, now_nanos):
    ...
    print(apply_steer, actuators.accel, enabled)
    # CAN controlled lateral running at 50hz
    if (self.frame % 2) == 0:
      can_sends.append(create_can_steer_command(self.packer, apply_steer, enabled, CS.hand_on_wheel_warning and CS.is_icc_on, (self.frame/2) % 16, CS.stock_lks_settings))
      ...
      can_sends.append(create_acc_cmd(self.packer, actuators.accel, enabled, (self.frame/2) % 16))

    # if long is controlled, standstill code is not needed
