...
def apply_std_steer_angle_limits(apply_angle, apply_angle_last, v_ego, LIMITS, log_file=None):
  steer_up = apply_angle_last * apply_angle >= 0. and abs(apply_angle) > abs(apply_angle_last)
  log_file.write(f'steer_direction up = {steer_up}\n')
  rate_limits = LIMITS.ANGLE_RATE_LIMIT_UP if steer_up else LIMITS.ANGLE_RATE_LIMIT_DOWN
  if LIMITS.single_angle_rate_limit_mode:
    rate_limits = LIMITS.ANGLE_RATE_LIMIT_UP if steer_up else LIMITS.ANGLE_RATE_LIMIT_DOWN
    output_angle = clip(apply_angle, apply_angle_last - rate_limits, apply_angle_last + rate_limits)
    if abs(output_angle - apply_angle) > 0.1:
      log_file.write(f'single_angle_rate {rate_limits} from in: {apply_angle} out: {output_angle}\n')
  else:
    angle_rate_lim = interp(v_ego, rate_limits.speed_bp, rate_limits.angle_v)
    output_angle = clip(apply_angle, apply_angle_last - angle_rate_lim, apply_angle_last + angle_rate_lim)
    if abs(output_angle - apply_angle) > 0.1:
      log_file.write(f'v_ego = {v_ego}\n angle_rate_lim = {angle_rate_lim}\n apply angle limits from in: {apply_angle} out: {output_angle}\n')
