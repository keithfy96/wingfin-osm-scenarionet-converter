...
excluded_interfaces = ["mock", "mazda", "tesla", "proton"]
...
def run_test_process(data):
  ...
  if not args.upload_only:
    ...
    log_paths = sorted([str(p) for p in Path("/log").rglob("rlog") if p.is_file()])
    lr = MultiLogIterator(log_paths)
...
def get_log_data(segment):
  ...
  url = get_url(r, n)
  file_path = os.path.join('rlogs', get_url(r, n).split(BASE_URL)[1])
...
def test_process(cfg, lr, segment, ref_log_path, new_log_path, ignore_fields=None, ignore_msgs=None):
  ...
  try:
    log_msgs = replay_process(cfg, lr, disable_progress=False)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--should-run-single-rlogs-test", action="store_true")
  ...
  with concurrent.futures.ProcessPoolExecutor(max_workers=args.jobs) as pool:
    ...
    if args.should_run_single_rlogs_test:
      filename = '/openpilot/selfdrive/test/process_replay/fakedata/678edf66f4b082a2---2023-10-19--11-51-01--2---rlog.bz2'
      with FileReader(filename) as f:
        ret = f.read()

      pool_args = [pool_args[2]]
      pool_args[0] = (pool_args[0][0], pool_args[0][1], pool_args[0][2], filename, None, ret)

      results: Any = defaultdict(dict)
      run_test_process(pool_args[0])
    else:
      results: Any = defaultdict(dict)
