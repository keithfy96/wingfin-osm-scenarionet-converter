...
CONFIGS = [
  ProcessConfig(
    ...
    pubs=[
      ...
      "longitudinalPlan", "lateralPlan", "liveLocationKalman", "liveParameters",
...
def _replay_single_process(cfg, lr, fingerprint, disable_progress):
  with OpenpilotPrefix():
    ...
    with ReplayContext(cfg) as rc:
      ...
      print(f'start process {cfg.proc_name} initiated')

      if cfg.init_callback is not None:
        cfg.init_callback(rc, pm, all_msgs, fingerprint)

      print('finished cfg.init_callback')

      log_msgs, msg_queue = [], []
      try:
        print('Waiting for the process to start...')
        # Wait for process to startup
        with Timeout(9999, error_msg=f"timed out waiting for process to start: {repr(cfg.proc_name)}"):
          while not all(pm.all_readers_updated(s) for s in cfg.pubs):
            for s in cfg.pubs:
              print(f'{s} {pm.all_readers_updated(s)}')
