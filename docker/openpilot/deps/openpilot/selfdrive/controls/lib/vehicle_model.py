...
class VehicleModel:
  ...
  def get_steer_from_curvature(self, curv: float, u: float, roll: float) -> float:
    ...
    """
    print(f'get_steer_from_curvature: curv = {curv} u = {u} roll = {roll}')
    print(f'    sR = {self.sR}')
    print(f'    curvature_factor = {self.curvature_factor(u)}')
  ...
  def roll_compensation(self, roll: float, u: float) -> float:
    ...
    print(f'roll_compensation: roll = {roll} u = {u} sf = {sf}')

    if abs(sf) < 1e-6:
      print('    return 0')
      return 0
    else:
      ret = (ACCELERATION_DUE_TO_GRAVITY * roll) / ((1 / sf) - u**2)
      print(f'    ret = {ret}')
      return ret
...
def calc_slip_factor(VM: VehicleModel) -> float:
  ...
  """
  print(f'calc_slip_factor: m = {VM.m} cF = {VM.cF} aF = {VM.aF} cR = {VM.cR} aR = {VM.aR} l = {VM.l}')
