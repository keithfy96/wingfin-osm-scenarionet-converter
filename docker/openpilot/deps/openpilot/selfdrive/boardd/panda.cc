...
bool Panda::unpack_can_buffer(uint8_t *data, int size, std::vector<can_frame> &out_vec) {
  ...
  while (pos < recv_buf.size()) {
    can_header header;
    memcpy(&header, &recv_buf[pos], 5);
    //const uint8_t data_len_rx = dlc_to_len[header.data_len_code];
    
    //if (calculate_checksum(&data[pos], sizeof(can_header) + data_len_rx) != 0) {
    //  LOGE("Panda CAN checksum failed");
    //  size = 0;
    //  return false;
    //}
    //if (header.addr > 2015) {
    //  LOGE("invalid addr %d", header.addr);
    //  size = 0;
    //  return false;
    //}
    ...
    if (header.rejected) { canData.src += CAN_RETURNED_BUS_OFFSET; }
    if (header.returned) { canData.src += CAN_RETURNED_BUS_OFFSET; }

    const uint8_t data_len = dlc_to_len[header.data_len_code];
    canData.dat.assign((char *)&recv_buf[pos + 5], data_len);
