...
def _audit_map_data(source_path: Path, audit: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    ...
    searchable_node_features = [
        _feature(
            Point(node.longitude, node.latitude),
            {
                "audit_issue": "node_search",
                "osm_id": node.identifier,
                "feature_type": "node",
                "tags": node.tags,
            },
    ...
    return {
        ...
        "searchable_nodes": _feature_collection(searchable_node_features),
    }
...
const searchableWays=new Map(data.searchable_ways.features.map(feature=>[String(feature.properties.osm_id),feature])); const searchableNodes=new Map(data.searchable_nodes.features.map(feature=>[String(feature.properties.osm_id),feature])); const missingRestrictionWays=new Set(audit.turn_restrictions.details.flatMap(relation=>relation.missing_way_member_ids)); let searchHighlight=null; const searchResult=document.getElementById('osm-search-result'); document.getElementById('osm-search').addEventListener('submit',event=>{{event.preventDefault();const elementType=document.getElementById('osm-type').value;const osmId=document.getElementById('osm-id').value.trim();if(searchHighlight){{map.removeLayer(searchHighlight);searchHighlight=null}}if(!/^\\d+$/.test(osmId)){{searchResult.textContent=`Enter a numeric OSM ${{elementType==='way'?'Way':'Node'}} ID.`;return}}const feature=(elementType==='way'?searchableWays:searchableNodes).get(osmId);if(!feature){{searchResult.textContent=elementType==='way'&&missingRestrictionWays.has(osmId)?`Way ${{osmId}} is referenced by a restriction but is missing from source/map.osm.`:`${{elementType==='way'?'Way':'Node'}} ${{osmId}} is not present${{elementType==='way'?' with drawable geometry':''}} in source/map.osm.`;return}}if(elementType==='way'){{const outline=L.geoJSON(feature,{{style:{{color:'#111',weight:12,opacity:1}}}});const focus=L.geoJSON(feature,{{style:{{color:'#ffe600',weight:7,opacity:1}},onEachFeature:(f,l)=>l.bindPopup(popup(f))}});searchHighlight=L.layerGroup([outline,focus]).addTo(map);const focusBounds=focus.getBounds();if(focusBounds.isValid())map.fitBounds(focusBounds.pad(.35),{{maxZoom:18}});focus.eachLayer(layer=>layer.openPopup());const p=feature.properties,tags=p.tags||{{}};searchResult.textContent=`Found Way ${{osmId}}: ${{tags.name||tags.highway||'unnamed way'}} (${{p.selection_reason}}). Highlighted yellow.`;return}}const focus=L.geoJSON(feature,{{pointToLayer:(f,ll)=>L.circleMarker(ll,{{radius:10,color:'#111',weight:3,fillColor:'#ffe600',fillOpacity:1}}),onEachFeature:(f,l)=>l.bindPopup(popup(f))}});searchHighlight=focus.addTo(map);const coordinates=feature.geometry.coordinates;map.setView([coordinates[1],coordinates[0]],19);focus.eachLayer(layer=>layer.openPopup());const tags=feature.properties.tags||{{}};const description=tags.name||tags.highway||tags.traffic_signals||Object.keys(tags).length+' source tag(s)';searchResult.textContent=`Found Node ${{osmId}}: ${{description}}. Highlighted yellow.`}});
    ...
    if view == "audit":
        ...
        inspection_report = {
            ...
            "layers": {
                ...
                if name not in {"searchable_ways", "searchable_nodes"}
            },
            "search": {
                "indexed_source_way_count": len(data["searchable_ways"]["features"]),
                "indexed_source_node_count": len(data["searchable_nodes"]["features"]),
            },
            "coverage": {
                "source_review": [
                    "lane_count",
