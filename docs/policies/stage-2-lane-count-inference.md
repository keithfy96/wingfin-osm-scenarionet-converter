...
The problem with `lanes=3` is that arithmetic cannot determine where the third lane belongs. It could be:
...
- 78 even totals inferred one lane per direction;
- six even totals inferred two lanes per direction;
- four low-confidence total cases inferred one lane;
- total: 638 findings across 117 source ways;
- 554 blockers and 84 warnings.
